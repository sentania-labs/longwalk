...
The peer sign-off is separate from the external Codex review on the round PR.
Neither substitutes for the other. The external reviewer is a separate
identity, not this worker seat.
...
PR branch). A `codex/*` branch appearing on `origin` is a rule violation that

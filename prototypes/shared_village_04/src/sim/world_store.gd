...
func initialize(path: String, version: String) -> Dictionary:
	...
	if names.is_empty(): return {"schema":1,"baseline":baseline,"revision":0,"players":{},"owner":"","cut":false,"config":{"save_interval":5.0,"max_players":4},"paused":false}
...
func valid_state(state: Variant) -> bool:
	...
	if state.get("schema") != 1 or state.get("baseline") != baseline: return false

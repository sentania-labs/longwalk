...
func _ready() -> void:
	...
	if server:
		...
		return
	if bot != "":
		var runner: Node = load("res://test/network_bot.gd").new()
		runner.link = link
		runner.scenario = bot
		add_child(runner)
		return
...
func _build_panel() -> void:
	...
	var title := Label.new()
	title.text = "SHARED VILLAGE 03a | Connection"
	box.add_child(title)
...
func _connect() -> void:
	...
	_save_client()
	link.connect_client(address.text.strip_edges(),credentials[profiles[profile.selected]])

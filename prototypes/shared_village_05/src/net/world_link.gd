...
const PROTOCOL = "shared-village-05"
...
func hello(baseline: String, credential: String) -> void:
	...
	if baseline != version + "|" + PROTOCOL:
		answer.rpc_id(peer,{"message":"Version mismatch: this server requires Shared Village 05 with the matching world baseline."})
		return
...
func request(action: String, arguments: Dictionary) -> void:
	...
	match action:
		...
		"workshop":
			...
			if message == "":
				if _commit(): message = "Saved: " + op.capitalize()
...
func _push_state(peer: int = 0) -> void:
	...
	for ident in connections.values():
		actors.append({"id":ident,"position":world.data.players[ident].position,"area":world.data.players[ident].get("area","outside"),"moving":world.routes.has(ident),"running":world.running.get(ident,false)})
	var payload := {"clock":world.data.get("clock",900),"sequence":sequence,"boot":boot_id,"actors":actors,"cut":world.data.cut,"revision":world.data.revision,"saved_generation":store.generation,"paused":world.data.paused or frozen,"error":error,"config":world.data.config,"backups":store.records("backup-")}
	var recipients: Array = [peer] if peer else connections.keys()
	for id in recipients:
		payload.person = world.data.players[connections[id]].duplicate(true)
		state.rpc_id(id,payload)

...
func _run() -> void:
	...
	link.multiplayer.connected_to_server.connect(func(): link.hello.rpc_id(1,link.version+"|shared-village-04","0".repeat(64)))
	link.notice.connect(func(message):
		if message.begins_with("Version mismatch:"):
			assert(not link.connected)
			print("PROTOCOL PASSED: 04 client rejected with upgrade message before registration")
			quit()
	)
	link.connect_client("127.0.0.1","0".repeat(64),17779)

...
const COMMUNITY = preload("res://src/sim/community.gd")
const LAND = preload("res://src/sim/landscape.gd")
const AREA := 512.0
const CELL := 1.0
var sun: DirectionalLight3D
var atmosphere: Environment
var sky_colors: ProceduralSkyMaterial
var camera: Camera3D
var avatar: Node3D
...
var remote_actors := {}
var traveler_names := true
var names_toggle: CheckButton
var resource_cut := false
var area := "outside"
var room: Node3D
var provisioner: Node3D
var provisioner_label: Label3D
var woodfalls := {}
var moth_root: Node3D
var moths := {}
var outside_nodes := []
var show_route := false
var marker_time := 0.0
var shortcut_move := true
var route_toggle: CheckButton
var shortcut_toggle: CheckButton
var room_nav = preload("res://src/sim/shared_world.gd").room_grid()
var pick_records := []
...
func _ready() -> void:
	...
	_build_context()
	_build_community()
	moth_root = Node3D.new()
	add_child(moth_root)
	for node in get_children():
		if node is Node3D and node != avatar and node != camera and node != marker and node != route_line and not node is Light3D: outside_nodes.append(node)
	room = preload("res://workshop_room.gd").new()
	room.village = self
	add_child(room)
	room.hide()
...
func _build_world() -> void:
	...
	_normalize_character_materials(model)
...
func _route_to(point: Vector3) -> Array[Vector3]:
	var grid = room_nav if area == "workshop" else nav
	var result: Array[Vector3] = []
	var end := _cell(point)
	var start := _cell(avatar.position)
	if not grid.is_in_boundsv(end) or grid.is_point_solid(end) or not grid.is_in_boundsv(start): return result
	for id in grid.get_id_path(start, end):
...
func _walk_to(point: Vector3, run := false) -> bool:
	if not network or not network.connected:
		...
		return false
	marker.position = point+Vector3(0,0.08,0)
	if area == "outside": marker.position.y = _ground_height(point)+0.08
	marker.visible = true
	marker_time = 1.6
...
func _draw_route(points: Array[Vector3]) -> void:
	route_line.visible = show_route
	if not show_route: return
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if event.physical_keycode == keys.center:
			...
			if get_parent().get("hud") and get_parent().hud.close_panels():
				get_viewport().set_input_as_handled()
				return
			if inspecting: _set_inspection(false)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		...
		elif event.pressed and event.button_index == MOUSE_BUTTON_LEFT and shortcut_move:
...
func _update_edge_scroll(delta: float) -> void:
	...
	if edge_enabled and not editing_text and not inspecting and not capture_mode and not dragging and get_window().has_focus() and get_viewport().gui_get_hovered_control() == null:
...
func _build_ui() -> void:
	...
	camera_controls.add_child(camera_column)
	var camera_head := HBoxContainer.new()
	camera_column.add_child(camera_head)
	var camera_title := _label("Camera / controls",20)
	camera_title.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	camera_head.add_child(camera_title)
	_button("×",func(): camera_controls.hide(),camera_head)
	preload("res://floating_panel.gd").attach(camera_controls,camera_title,"Camera")
	camera_column.add_child(scroll)
	...
	names_toggle = CheckButton.new()
	names_toggle.text = "Show nearby traveler names"
	names_toggle.button_pressed = traveler_names
	names_toggle.toggled.connect(func(value): traveler_names=value; _save_settings())
	box.add_child(names_toggle)
	...
	_button("Reset window positions",func(): get_tree().call_group("floating_panels","reset_position"),box)
	...
	map_view.hide()
	status_label.hide()
	travel_controls.get_parent().remove_child(travel_controls)
	box.add_child(travel_controls)
	route_toggle = CheckButton.new()
	route_toggle.text = "Show route line (optional)"
	route_toggle.toggled.connect(func(value): show_route=value; route_line.visible=value; _save_settings())
	box.add_child(route_toggle)
	shortcut_toggle = CheckButton.new()
	shortcut_toggle.text = "Left-click walk / Shift-click run"
	shortcut_toggle.button_pressed = true
	shortcut_toggle.toggled.connect(func(value): shortcut_move=value; _save_settings())
	box.add_child(shortcut_toggle)
...
func _save_settings(path: String = SETTINGS_PATH) -> void:
	...
	cfg.set_value("camera","traveler_names",traveler_names)
	...
	cfg.set_value("travel","show_route",show_route)
	cfg.set_value("travel","shortcut_move",shortcut_move)
...
func _load_settings(path: String = SETTINGS_PATH) -> void:
	...
	var cfg := ConfigFile.new()
	if cfg.load(path) == OK:
		show_route = bool(cfg.get_value("travel","show_route",false))
		shortcut_move = bool(cfg.get_value("travel","shortcut_move",true))
		var candidate := {}
		var used := []
		var valid := true
		for action in DEFAULT_KEYS:
		...
		traveler_names = bool(cfg.get_value("camera","traveler_names",true))
...
func _sync_preferences() -> void:
	route_toggle.set_pressed_no_signal(show_route)
	shortcut_toggle.set_pressed_no_signal(shortcut_move)
	names_toggle.set_pressed_no_signal(traveler_names)
...
func _receive_world(state: Dictionary) -> void:
	var next_area: String = state.get("person",{}).get("area","outside")
	if next_area != area:
		area = next_area
		for node in outside_nodes: node.visible = area == "outside"
		room.visible = area == "workshop"
		context_panel.hide()
		var pos: Array = state.person.position
		avatar.position = Vector3(pos[0],pos[1],pos[2])
		focus = avatar.position+Vector3(0,1,0)
		focus_target = focus
		if inspecting: _set_inspection(false)
		zoom_slider.value = 22 if area == "workshop" else 48
		follow_traveler = true
	var wildlife: Dictionary = state.get("wildlife",{})
	var present_moths := []
	for insect in wildlife.get("agents",[]):
		present_moths.append(insect.id)
		if not moths.has(insect.id):
			var model: Node3D = load("res://lantern_moth.gd").new()
			moth_root.add_child(model)
			model.position = Vector3(insect.position[0],insect.position[1],insect.position[2])
			model.phase = float(insect.id)
			moths[insect.id] = model
	for id in moths.keys():
		if id not in present_moths: moths[id].queue_free(); moths.erase(id)
	moth_root.visible = area == "outside" and wildlife.get("active",false)
	for key in woodfalls:
		woodfalls[key].visible = area == "outside" and float(state.get("gathering",{}).get(key,0))<=float(state.get("elapsed",0))
	_update_daylight(float(state.get("clock",900)))
...
func _update_network_actors(delta: float) -> void:
	...
	if not network or not network.connected:
		...
		return
	for insect in network.snapshot.get("wildlife",{}).get("agents",[]):
		if not moths.has(insect.id): continue
		var model: Node3D = moths[insect.id]
		var target := Vector3(insect.position[0],insect.position[1],insect.position[2])
		var direction := target-model.position
		model.position = model.position.lerp(target,1-exp(-delta*5))
		if direction.length()>0.01: model.rotation.y = atan2(direction.x,direction.z)
	if provisioner and network.snapshot.has("citizen"):
		provisioner.visible = area == "outside"
		var npc: Dictionary = network.snapshot.citizen
		var target := Vector3(npc.position[0],npc.position[1],npc.position[2])
		var direction := target-provisioner.position
		provisioner.position = target if direction.length()>10 else provisioner.position.lerp(target,1-exp(-delta*12))
		if Vector2(direction.x,direction.z).length()>0.025: provisioner.rotation.y = lerp_angle(provisioner.rotation.y,atan2(direction.x,direction.z),1-exp(-delta*12))
		var anim: AnimationPlayer = _find_animation(provisioner)
		var clip := walk_clip if npc.moving else idle_clip
		if anim.current_animation != clip: anim.play(clip,0.15)
		provisioner_label.text = "Provisioner\n"+("Walking to "+npc.activity if npc.moving else "Bread · 1 coin" if npc.activity == "market" and target.distance_to(COMMUNITY.PLACES.market)<0.25 else "Off duty / waiting")
	for person in network.snapshot.get("actors",[]):
		...
		actor.visible = person.get("area","outside") == area
		if actor != avatar:
			var name_tag: Label3D = actor.get_node("TravelerName")
			name_tag.text = network.snapshot.get("names",{}).get(person.id,"Traveler")
			name_tag.visible = traveler_names and (inspecting or camera.size<60)
...
func _pick_context(screen: Vector2) -> Dictionary:
	if area == "workshop":
		...
		var closest := INF
		var object := ""
		for key in preload("res://src/sim/workshop.gd").POINTS:
			var base: Vector3 = preload("res://src/sim/workshop.gd").POINTS[key]
			var box := AABB(base-Vector3(0.9,0,0.7),Vector3(1.8,2.3,1.4))
			var contact = box.intersects_ray(ray_origin,ray_direction)
			if contact != null and ray_origin.distance_to(contact)<closest:
				closest=ray_origin.distance_to(contact)
				object=key
		if object != "": return {"kind":object,"name":object.capitalize(),"point":preload("res://src/sim/workshop.gd").POINTS[object]}
		var hit = _ground_hit(screen)
		if hit == null: return {}
		var picked := ""
		var nearest := 2.0
		for key in preload("res://src/sim/workshop.gd").POINTS:
			var distance: float = hit.distance_to(preload("res://src/sim/workshop.gd").POINTS[key])
			if distance < nearest: nearest=distance; picked=key
		if picked != "": return {"kind":picked,"name":picked.capitalize(),"point":preload("res://src/sim/workshop.gd").POINTS[picked]}
		return {"kind":"ground","name":"Workshop floor","point":hit}
	...
	if provisioner.visible:
		targets.append({"bounds":AABB(provisioner.position+Vector3(-0.6,0,-0.6),Vector3(1.2,2.3,1.2)),"action":{"kind":"provisioner","name":"Village provisioner","point":provisioner.position}})
	if moth_root.visible:
		for id in moths:
			targets.append({"bounds":AABB(moths[id].position-Vector3.ONE*0.4,Vector3.ONE*0.8),"action":{"kind":"lantern_moth","name":"Lantern moth","point":moths[id].position}})
	for key in woodfalls:
		targets.append({"bounds":AABB(COMMUNITY.PILES[key]+Vector3(-1,0,-1),Vector3(2,1.5,2)),"action":{"kind":"woodfall","pile":key,"name":key.capitalize()+" woodfall","point":COMMUNITY.PILES[key]}})
	for target in targets:
		var hit = target.bounds.intersects_ray(origin,direction)
		if hit != null and origin.distance_to(hit)<closest:
			closest = origin.distance_to(hit)
			dynamic = target.action
	for record in pick_records:
		...
		if hit != null and origin.distance_to(hit) < closest:
			...
			selected = record
			dynamic = {}
	if not dynamic.is_empty(): return dynamic
	...
	if selected.key == "barn" and selected.node.position.distance_to(Vector3(-102,0,-83))<1: kind="enter"
	return {"kind":kind,"name":"Carpenter's workshop" if kind == "enter" else kind.replace("_"," ").capitalize(),"point":selected.node.position}
...
func _refresh_context() -> void:
	...
	var person: Dictionary = network.snapshot.get("person",{}) if online else {}
	var snapshot: Dictionary = network.snapshot if online else {}
	var point: Vector3 = provisioner.position if context_selection.kind == "provisioner" else context_selection.point
	if context_selection.kind == "enter": point = preload("res://src/sim/workshop.gd").ENTRY
	var in_reach := avatar.position.distance_to(point)<= (3.0 if area == "outside" else 2.4)
	var recovered := float(snapshot.get("gathering",{}).get(context_selection.get("pile",""),0))<=float(snapshot.get("elapsed",0))
	var fingerprint := str([online,paused,nearby,resource_cut,in_reach,recovered,person.get("quest"),person.get("order"),person.get("inventory"),person.get("observations"),snapshot.get("citizen",{}).get("moving"),snapshot.get("citizen",{}).get("activity")])
	...
	var actions: Array = preload("res://context_actions.gd").choices(context_selection.kind,resource_cut,nearby,online and not paused)
	var verbs := {"enter":"Enter workshop","exit":"Leave workshop","wood":"Collect wood bundle","tool":"Take free starter axe","bench":"Prepare wood","trade":"Deliver wood for 5 coins + ring"}
	var can_act := online and not paused and in_reach
	var quest: Dictionary = person.get("quest",{})
	if context_selection.kind in verbs:
		var done: bool = (context_selection.kind == "wood" and quest.get("wood",false)) or (context_selection.kind == "tool" and quest.get("tool",false)) or (context_selection.kind == "bench" and quest.get("prepared",false)) or (context_selection.kind == "trade" and quest.get("complete",false))
		actions.append({"id":"workshop","label":"Already completed" if done else verbs[context_selection.kind]+(" (move closer)" if not in_reach else ""),"enabled":can_act and not done})
	if context_selection.kind == "wood": actions.append({"id":"observe","label":"Grain already studied" if "wood_grain" in person.get("observations",[]) else "Study wood grain","enabled":can_act and "wood_grain" not in person.get("observations",[])})
	if context_selection.kind == "lantern_moth": actions.append({"id":"observe_moths","label":"Watch the small hours","enabled":can_act})
	if context_selection.kind == "woodfall": actions.append({"id":"gather","label":"Trim fallen branches (+3 wood)" if recovered else "Branches recovering","enabled":can_act and recovered})
	if context_selection.kind == "provisioner":
		var trading: bool = snapshot.get("citizen",{}).get("activity","") == "market" and not snapshot.get("citizen",{}).get("moving",true) and provisioner.position.distance_to(COMMUNITY.PLACES.market)<0.25
		actions.append({"id":"buy_bread","label":"Buy bread · 1 coin" if trading else "Provisioner is off duty / traveling","enabled":can_act and trading and person.get("coins",0)>=1})
	if context_selection.kind == "trade" and quest.get("complete",false):
		actions.append({"id":"order","label":"Order already open" if person.get("order",false) else "Accept another order","enabled":can_act and not person.get("order",false)})
		actions.append({"id":"deliver_order","label":"Deliver order · 3 coins","enabled":can_act and person.get("order",false) and person.get("inventory",{}).get("prepared",0)>=6})
	if context_selection.kind == "bench" and person.get("order",false): actions.append({"id":"prepare_order","label":"Prepare order wood","enabled":can_act and person.get("inventory",{}).get("wood",0)>=6})
...
func _choose_context(action: String) -> void:
	...
	match action:
		"walk", "run":
			var destination: Vector3 = provisioner.position if context_selection.kind == "provisioner" else context_selection.point
			if context_selection.kind == "enter": destination = preload("res://src/sim/workshop.gd").ENTRY
			elif context_selection.kind != "ground": destination = _approach_point(destination)
			_walk_to(destination,action == "run")
		"gather": network.command("village",{"op":"gather","pile":context_selection.pile})
		"buy_bread", "order", "deliver_order", "prepare_order", "observe_moths": network.command("village",{"op":action})
		"workshop": network.command("workshop",{"op":context_selection.kind})
		"harvest": network.command("harvest")
		"observe": network.command("workshop",{"op":"observe","subject":"wood_grain"})
		"inspect":
			var descriptions := {"enter":"Carpenter's workshop. Walk to the door to enter.","wood":"Six pieces of oak wood set aside for your commission.","tool":"A free starter axe, yours to keep. Equip it to prepare the wood.","bench":"Equip the axe and bring six wood here to prepare them.","trade":"The carpenter exchanges your prepared wood for five coins and a copper ring.","exit":"Return to the West farms."}
			if context_selection.kind in descriptions:
				status_label.text = descriptions[context_selection.kind]
				return
			match context_selection.kind:
				"lantern_moth": status_label.text = "Lantern moth: a tiny nocturnal meadow creature. It feeds, roosts and breeds independently of your visit."
				"woodfall": status_label.text = "Fallen branches. Equip an axe and gather three wood. Shared piles recover after three minutes of running world time."
				"provisioner": status_label.text = "The village provisioner tends the west farm in the morning, sells bread at the market from late morning, and heads home at night."
				...
				"firewood": status_label.text = "A pile of village firewood. Your commission bundle is inside the workshop."
...
func _approach_point(point: Vector3) -> Vector3:
	...
	for x in range(-12,13):
		for z in range(-12,13):
			...
			if not grid.is_in_boundsv(id) or grid.is_point_solid(id): continue
			var occupied := false
			if network and network.connected:
				for actor in network.snapshot.get("actors",[]):
					if actor.id == network.local_id or actor.get("area","outside") != area: continue
					if Vector2(candidate.x,candidate.z).distance_to(Vector2(actor.position[0],actor.position[2]))<1.35: occupied=true
...
func _build_community() -> void:
	provisioner = Node3D.new()
	var person := Walker.instantiate()
	_normalize_character_materials(person)
	provisioner.add_child(person)
	add_child(provisioner)
	provisioner.position = COMMUNITY.PLACES.home
	var anim := _find_animation(provisioner)
	anim.add_animation_library("motion",animation.get_animation_library("motion"))
	anim.play(idle_clip)
	provisioner_label = Label3D.new()
	provisioner_label.text = "Village provisioner"
	provisioner_label.position.y = 2.4
	provisioner_label.font_size = 26
	provisioner_label.pixel_size = 0.008
	provisioner_label.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	provisioner.add_child(provisioner_label)
	for key in COMMUNITY.PILES:
		var holder := Node3D.new()
		add_child(holder)
		holder.position = COMMUNITY.PILES[key]
		var pile := ART.firewood.instantiate()
		holder.add_child(pile)
		var bounds := _bounds(pile,Transform3D.IDENTITY)
		pile.scale *= 0.8/bounds.size.y
		pile.position = -Vector3(bounds.get_center().x,bounds.position.y,bounds.get_center().z)*0.8/bounds.size.y
		woodfalls[key] = holder

func _update_daylight(clock: float) -> void:
	var solar := sin((clock-360)/1440.0*TAU)
	var daylight := smoothstep(-0.1,0.25,solar)
	sun.light_energy = 0.7*daylight
	sun.rotation_degrees.x = -maxf(8,solar*65)
	var warmth := 1-smoothstep(0.05,0.45,solar)
	sun.light_color = Color("fff0ce").lerp(Color("ffc184"),warmth)
	atmosphere.ambient_light_energy = lerpf(0.36,0.65,daylight)
	atmosphere.ambient_light_color = Color("788ba8").lerp(Color("dbe3d3"),daylight)
	sky_colors.sky_top_color = Color("152338").lerp(Color("799daf"),daylight)
	sky_colors.sky_horizon_color = Color("394b60").lerp(Color("c5cbb5"),daylight)
	sky_colors.ground_horizon_color = sky_colors.sky_horizon_color

func _normalize_character_materials(model: Node3D) -> void:

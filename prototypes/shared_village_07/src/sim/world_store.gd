...
func initialize(path: String, version: String) -> Dictionary:
	...
	if names.is_empty(): return {"schema":3,"baseline":baseline,"revision":0,"players":{},"owner":"","cut":false,"config":{"save_interval":5.0,"max_players":4},"paused":false}
...
func read_record(name: String) -> Dictionary:
	...
	if decoder.parse(FileAccess.get_file_as_string(directory.path_join(name))) != OK:
		error = "Invalid save envelope: " + name
		return {}
	var envelope = decoder.data
	...
	if decoder.parse(envelope.payload) != OK:
		error = "Invalid world payload: " + name
		return {}
	var state = decoder.data
...
func valid_state(state: Variant) -> bool:
	...
	if not preload("res://src/sim/community.gd").valid(state): return false
	if not preload("res://src/sim/lantern_moths.gd").valid(state): return false
	if (state.get("schema") != 1 and state.get("schema") != 2 and state.get("schema") != 3) or state.get("baseline") != baseline: return false
	...
	if cfg.has("day_minutes") and (not preload("res://src/sim/community.gd")._number(cfg.day_minutes) or cfg.day_minutes<12 or cfg.day_minutes>1440): return false
	...
	for ident in state.players:
		...
		if state.schema == 3 and (not person.has("practice") or not person.inventory.has("prepared")): return false

...
const DEFAULT_KEYS = {"forward":KEY_W,"back":KEY_S,"left":KEY_A,"right":KEY_D,"rotate_left":KEY_Q,"rotate_right":KEY_E,"zoom_in":KEY_MINUS,"zoom_out":KEY_EQUAL,"center":KEY_SPACE,"inspect":KEY_TAB,"reset_view":KEY_R}
...
func _walk_to(point: Vector3, run := false) -> bool:
	var candidate := _route_to(point)
	if candidate.is_empty():
		status_label.text = "No route there. Use open ground and field gateways."
		return false
	running = run
	route = candidate
	preview_route = candidate.duplicate()
	_draw_route(preview_route)
	marker.position = route.back() + Vector3(0, 0.05, 0)
	marker.visible = true
	status_label.text = ("Running" if running else "Walking") + (" via paths." if nav.prefer_paths else " via a direct route.")
	return true
...
func _update_preview(delta: float) -> void:
	preview_timer -= delta
	if capture_mode or dragging or not get_window().has_focus() or preview_timer > 0: return
	preview_timer = 0.3
...
func _process(delta: float) -> void:
	if follow_traveler and not inspecting and not route.is_empty():
	...
	if value:
		...
		for radius in range(0, 20):
			for angle in range(0, 360, 30):
				var candidate := start + Vector3(sin(deg_to_rad(angle)),0,cos(deg_to_rad(angle))) * radius
				var id := _cell(candidate)
				if nav.is_in_boundsv(id) and not nav.is_point_solid(id):
					inspection_position = candidate
					inspection_position.y = _ground_height(candidate) + 1.7
					found = true
					break
			if found: break
		if not found:
			inspection_position = avatar.position + Vector3(0,1.7,0)
		...
		camera.fov = 65
		status_label.text = "Inspect: WASD moves, right-drag looks up/down. Tab returns overhead; Space returns to traveler."
	else:
...
func _apply_pan_input(direction: Vector2, delta: float) -> void:
	...
	if inspecting:
		var motion := pan_velocity * delta
		var steps := maxi(1, ceili(motion.length() / 0.25))
		for i in range(steps):
			var candidate := inspection_position + motion / steps
			var id := _cell(candidate)
			if nav.is_in_boundsv(id) and not nav.is_point_solid(id):
				inspection_position = candidate
		inspection_position.y = _ground_height(inspection_position) + 1.7
...
func _build_ui() -> void:
	...
	heading.add_child(_label("TWO RIVERS  /  COMFORT 02  /  1 km²", 14, Color("e8d4a7")))
...
func _capture_study(path:String) -> void:
	...
	passed = await _comfort_checks(path) and passed
	if passed: print("TWO RIVERS COMFORT CHECKS PASSED: 1024m map, %d buildings, %d trees, farm routes, bridge-only water crossings, run animation/speed, keyboard pan"%[building_count,tree_count])
	get_tree().quit(0 if passed else 1)


func _refresh_bindings() -> void:
	if controls_label:
		controls_label.text = "Click: move · Shift-click: run · Hold %s/%s: rotate · %s: in / %s: out · %s: inspect" % [OS.get_keycode_string(keys.rotate_left),OS.get_keycode_string(keys.rotate_right),OS.get_keycode_string(keys.zoom_out if reverse_zoom else keys.zoom_in),OS.get_keycode_string(keys.zoom_in if reverse_zoom else keys.zoom_out),OS.get_keycode_string(keys.inspect)]
...
func _test_key(code: int, pressed: bool) -> void:
...
func _comfort_checks(path: String) -> bool:
	var passed := true
	get_window().grab_focus()
	await get_tree().create_timer(0.2).timeout
	_set_follow(false)
	# Drive the actual held-key handler at two tick sizes, then release.
	var initial := yaw
	capture_mode = false
	_test_key(KEY_E, true)
	for i in range(10): _update_keyboard_rotation(0.05)
	_test_key(KEY_E, false)
	var after := yaw
	_update_keyboard_rotation(0.1)
	passed = _check(absf(after-initial-rotation_speed*0.5)<0.01,"Held E must turn continuously at configured speed") and passed
	passed = _check(yaw == after,"Released E must stop turning") and passed
	var size_before := camera.size
	_test_key(KEY_MINUS,true)
	_update_keyboard_rotation(0.1)
	_test_key(KEY_MINUS,false)
	passed = _check(camera.size<size_before,"Minus must zoom in") and passed
	_test_key(KEY_EQUAL,true)
	_update_keyboard_rotation(0.1)
	_test_key(KEY_EQUAL,false)
	passed = _check(absf(camera.size-size_before)<0.1,"Equals must zoom out") and passed
	capture_mode = true
	# Check deliberate route choice against the actual populated village.
	avatar.position = Vector3(0,0,165)
	nav.prefer_paths = true
	var normal := _route_to(Vector3(60,0,245))
	nav.prefer_paths = false
	var direct := _route_to(Vector3(60,0,245))
	passed = _check(not normal.is_empty() and direct.size()<normal.size(),"Direct choice must permit a shorter route") and passed
	nav.prefer_paths = true
	for i in range(LAND.GATES.size()):
		var gate: Vector2 = LAND.GATES[i]
		var center: Vector2 = LAND.FARMS[i].get_center()
		var outside := gate + (gate-center).normalized()*6
		avatar.position = Vector3(outside.x,0,outside.y)
		var field_route := _route_to(Vector3(center.x,0,center.y))
		passed = _check(not field_route.is_empty(),"Populated scene field access %d" % i) and passed
		for step in field_route:
			passed = _check(not nav.is_point_solid(_cell(step)),"Field route crossed an obstacle") and passed
	avatar.position = Vector3(-165,0,-77)
	preview_route = _route_to(Vector3(-123,0,-48))
	_draw_route(preview_route)
	focus = Vector3(-125,1.2,-55)
	focus_target = focus
	yaw = 45
	target_yaw = yaw
	zoom_slider.value = 65
	_update_camera()
	await RenderingServer.frame_post_draw
	get_viewport().get_texture().get_image().save_png(path.path_join("gate-route.png"))
	# Exercise a real ground-level barn view, then pitch above the horizon.
	focus = Vector3(-102,1.2,-74)
	focus_target = focus
	yaw = 0
	target_yaw = yaw
	_set_inspection(true)
	elevation_slider.value = -24
	_update_camera()
	passed = _check(camera.projection==Camera3D.PROJECTION_PERSPECTIVE and camera.position.y>=1.7,"Inspection camera above ground") and passed
	passed = _check((-camera.global_basis.z).y>0,"Inspection camera can look upward") and passed
	await RenderingServer.frame_post_draw
	get_viewport().get_texture().get_image().save_png(path.path_join("barn-look-up.png"))
	# Test heading-relative movement without crossing the building footprint.
	inspection_position = Vector3(-102,1.7,-74)
	pan_velocity = Vector3.ZERO
	var before := inspection_position
	for i in range(100): _apply_pan_input(Vector2(0,-1),0.05)
	passed = _check(inspection_position.z < before.z,"Inspection W advances toward camera heading") and passed
	passed = _check(not nav.is_point_solid(_cell(inspection_position)),"Inspection movement cannot pass into barn") and passed
	passed = _check(inspection_position.z > -79,"Inspection movement stops at barn frontage") and passed
	_center_traveler()
	passed = _check(not inspecting and follow_traveler and camera.projection==Camera3D.PROJECTION_ORTHOGONAL,"Center returns to overhead traveler") and passed
	# Rebinding rejects an occupied key and accepts a distinct key.
	binding_action = "forward"
	var key_event := InputEventKey.new()
	key_event.pressed = true
	key_event.physical_keycode = KEY_E
	_input(key_event)
	passed = _check(keys.forward==KEY_W and binding_action=="forward","Duplicate binding must be rejected") and passed
	key_event.physical_keycode = KEY_I
	_input(key_event)
	passed = _check(keys.forward==KEY_I and binding_action=="","Distinct binding must be accepted") and passed
	# Save/reload to a separate test file, leaving real preferences alone.
	keys.forward = KEY_I
	edge_speed = 11
	rotation_speed = 83
	nav.prefer_paths = false
	_save_settings("user://comfort-check.cfg")
	keys = DEFAULT_KEYS.duplicate()
	edge_speed = 7
	_load_settings("user://comfort-check.cfg")
	passed = _check(keys.forward==KEY_I and edge_speed==11 and rotation_speed==83 and not nav.prefer_paths,"Preferences survive reload") and passed
	_reset_preferences()
	focus = Vector3(0,1.2,5)
	focus_target = focus
	yaw = 45
	target_yaw = yaw
	...
	_update_camera()
	await RenderingServer.frame_post_draw
	get_viewport().get_texture().get_image().save_png(path.path_join("low-horizon.png"))
	elevation_slider.value = 35.264
	camera_controls.show()
	_update_camera()
	await RenderingServer.frame_post_draw
	get_viewport().get_texture().get_image().save_png(path.path_join("camera-settings.png"))
	return passed

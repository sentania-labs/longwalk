...
func _plan(ident: String) -> bool:
	...
	for occupied in blockers(ident):
...
func tick(delta: float, replan_limit: int = 1) -> void:
	moved.clear()
	if data.paused: return
	community.tick(self,delta)
	fauna.tick(data,delta)
	data.clock = fposmod(float(data.get("clock",900)) + delta*24/float(data.get("config",{}).get("day_minutes",144)),1440)
	var replans := 0
	...
	for ident in ordered:
		...
		if person.get("stamina",100)<=0: running[ident] = false
		var sprint: bool = running.get(ident,false) and person.get("stamina",100)>0
		var speed: float = WORK.SKILLS.run_speed(person) if sprint else 1.8
		var budget := speed*delta
		if sprint: budget = minf(budget,person.stamina/WORK.SKILLS.run_cost(person)*speed)
		var traveled := 0.0
		while not route.is_empty() and budget>0:
			...
			if not _can_step(ident,p,next):
				...
				if retry_after[ident] <= 0 and replans<replan_limit:
					replans+=1
					_plan(ident)
		...
		if traveled>0.00001: moved[ident] = true
		var moving_time := traveled/speed
		person.stamina = clampf(person.stamina + (-WORK.SKILLS.run_cost(person) if sprint else 0.5)*moving_time,0,WORK.SKILLS.stamina_max(person))
		if sprint and person.stamina<0.000001:
			person.stamina = 0
			running[ident] = false
		if sprint: person.practice.running = minf(100000000,person.practice.running+traveled)
		data.players[ident].position = [p.x,p.y,p.z]
...
func blockers(ident: String) -> Array[Vector3]:
	var points: Array[Vector3] = []
	for other in active:
		if other != ident and data.players.has(other) and data.players[other].get("area","outside") == data.players[ident].get("area","outside"): points.append(position(other))
	if data.has("citizen") and data.players[ident].get("area","outside") == "outside": points.append(COMMUNITY.position(data))
	return points

func village_action(ident: String, op: String, args: Dictionary) -> String:
	if data.paused: return "World is paused."
	return COMMUNITY.apply(self,ident,op,args)
